...
Bigroute does not store router dashboard passwords or authenticate to private web panels. Scheduled refreshes are read-only. Account mutations require an explicit user action: the manual account toggles (Enable All, Disable Inactive) or JSON credential import via CLI Proxy API `/v0/management/auth-files`. Upgrading from an older build removes retired automation credentials and ownership markers without sending a request to the router.
