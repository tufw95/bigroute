...
Bigroute stores provider API keys in macOS Keychain. Per-user quota snapshots contain display data but never API keys. When Bridge is enabled, its Node process reads a separate API-key configuration file with permissions 0600; disabling Bridge removes that file. Release signing certificates, provisioning profiles, notarization passwords, and the Sparkle EdDSA private key must remain in protected GitHub Actions secrets and must never be committed.

Bigroute does not store 9Router dashboard passwords or authenticate to dashboard management endpoints. Scheduled refreshes are read-only. Account mutations require a visible user action: the two 9Router account toggles or JSON import. For account toggles, the server selects candidates from its latest server-owned quota snapshot, rejects stale data, and reloads current account state before applying a change. Upgrading from an older build removes retired automation credentials and ownership markers without sending a request to the router.
...
The legacy `com.routerquota.*` bundle IDs, `group.com.routerquota.shared` App Group, Keychain service, Sparkle public key, and `Router Quota Office Signing` certificate name are compatibility identifiers. They must not be globally renamed or replaced during a product rebrand because existing installations rely on signing and credential continuity. The App Group is retained in signing configuration; WidgetKit and shared snapshot writes have been removed.
